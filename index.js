...
let totalContributions = GAMES_JSON.reduce((total, games) => {
...
function filterFundedOnly() {
    ...
    let fundedGames = GAMES_JSON.filter((game) => {
        return Number(game.pledged) >= Number(game.goal);
    });
...
const unfundedGamesString = unfundedGamesCount === 0 ? "all of our games are funded" : `we have ${unfundedGamesCount} unfunded games`;
const raisedString = totalRaised.toLocaleString();
const displayString = `At our company, ${unfundedGamesString} and we have raised ${raisedString} from ${totalContributions.toLocaleString()} backers.`;
...
secondGameElement.innerHTML = runnerUp.name;
